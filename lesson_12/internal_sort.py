...
def mprint(lst, variant):
    ...
    template_2 = '{full_name:<30}{grades}{summa:>6}'
    template_3 = '{full_name:<30}{grades}{summa:>6}{avg:>8}'
    grades = '{grade:<4}'
    ...
    for line in lst:
        ...
        if variant == 1:
            print(template_1.format(
                ...
                #  lst[1:][i] if i < len(lst[1:]) else '_' for i in range(max_cnt)
            ))
        elif variant == 2:
...
rating = [name + ', ' + ', '.join([str(randint(1, 12)) for _ in range(randint(15, 20))]) for name in names]
# pprint(rating)
mprint(rating, 1)
print()
...
rating.sort(key=lambda element: int(element.split(', ')[-2]))
mprint(rating, 3)
print()

# сортируем по среднему баллу
rating.sort(key=lambda element: round(
    sum(int(value) for value in element.split(', ')[1:-2])/len(element.split(', ')[1:-2]), 2), reverse=True)
mprint(rating, 3)
print()

# используем для сортировки метод sorted()
n = sorted(names, key=lambda full_name: full_name.split()[1])
pprint(n)
print()

# сортируем по длине имени
n = sorted(names, key=lambda full_name: len(full_name.split()[1]))
pprint(n)
print()
